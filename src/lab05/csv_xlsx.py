...
def csv_to_xlsx(csv_path: str, xlsx_path: str) -> None:
    ...
    for col_idx, col_cells in enumerate(
        ws.columns, start=1
    ):  # ws.columns - генератор всех колонок листа, enumerate(..., start=1) - получаем индекс колонки
        max_length = max(
            len(str(cell.value)) if cell.value is not None else 0 for cell in col_cells
        )  # для каждой колонки находим максимальную длину текста, находим максимальную длину среди всех ячеек колонки
        adjusted_width = max(max_length, 8)  # устанавливаем ширину как максимум
        col_letter = ws.cell(
            row=1, column=col_idx
        ).column_letter  # получаем букву колонки из первой ячейки колонки
        ws.column_dimensions[col_letter].width = (
            adjusted_width  # устанавливаем вычисленную ширину для колонки
        )
    wb.save(xlsx_file)
